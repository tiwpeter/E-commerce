...
export function AddressDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Address", config));
    }
    ...
    const pageTitle = config.docsTitle || `Address API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Address", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Address", config);
