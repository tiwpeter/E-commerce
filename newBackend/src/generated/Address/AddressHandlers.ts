...
export async function AddressFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).address.findMany(query);
    }
...
export async function AddressFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.findFirst(query);
    }
...
export async function AddressFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.findUnique(query);
    }
...
export async function AddressFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.findUniqueOrThrow(query);
    }
...
export async function AddressFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.findFirstOrThrow(query);
    }
...
export async function AddressCount(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).address.count(query);
    }
...
export async function AddressAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.aggregate(query);
    }
...
export async function AddressGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.groupBy(query);
    }
...
export async function AddressCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).address.create(body);
    }
...
export async function AddressCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.createMany(body);
    }
...
export async function AddressCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.createManyAndReturn(body);
    }
...
export async function AddressUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).address.update(body);
    }
...
export async function AddressUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.updateMany(body);
    }
...
export async function AddressUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.updateManyAndReturn(body);
    }
...
export async function AddressDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).address.delete(body);
    }
...
export async function AddressDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).address.deleteMany(body);
    }
...
export async function AddressUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).address);
      const caller = res.locals.guardCaller;
      data = await (extended as any).address.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).address.upsert(body);
    }
...
export async function AddressFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.address.guard(shape, caller).findMany(query)
            : await tx.address.findMany(query);
          const t = await countForPagination(tx.address, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).address
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).address.findMany(query);
          total = await countForPagination(
            (extended as any).address,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).address.guard(shape, caller).findMany(query)
        : await (extended as any).address.findMany(query);
      total = await countForPagination(
        (extended as any).address,
        query,
