...
const COMPOUND_UNIQUES: { name: string; fields: string[] }[] = [
  { name: "optionId_value", fields: ["optionId", "value"] },
];
...
export function OptionValueDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("OptionValue", config));
    }
    ...
    const pageTitle = config.docsTitle || `OptionValue API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("OptionValue", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("OptionValue", config);
