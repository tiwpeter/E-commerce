...
export function PaymentDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Payment", config));
    }
    ...
    const pageTitle = config.docsTitle || `Payment API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Payment", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Payment", config);
