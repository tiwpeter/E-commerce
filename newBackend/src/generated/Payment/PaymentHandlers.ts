...
export async function PaymentFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).payment.findMany(query);
    }
...
export async function PaymentFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.findFirst(query);
    }
...
export async function PaymentFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.findUnique(query);
    }
...
export async function PaymentFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.findUniqueOrThrow(query);
    }
...
export async function PaymentFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.findFirstOrThrow(query);
    }
...
export async function PaymentCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).payment.count(query);
    }
...
export async function PaymentAggregate(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.aggregate(query);
    }
...
export async function PaymentGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.groupBy(query);
    }
...
export async function PaymentCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).payment.create(body);
    }
...
export async function PaymentCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.createMany(body);
    }
...
export async function PaymentCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.createManyAndReturn(body);
    }
...
export async function PaymentUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).payment.update(body);
    }
...
export async function PaymentUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.updateMany(body);
    }
...
export async function PaymentUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.updateManyAndReturn(body);
    }
...
export async function PaymentDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).payment.delete(body);
    }
...
export async function PaymentDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).payment.deleteMany(body);
    }
...
export async function PaymentUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).payment);
      const caller = res.locals.guardCaller;
      data = await (extended as any).payment.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).payment.upsert(body);
    }
...
export async function PaymentFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.payment.guard(shape, caller).findMany(query)
            : await tx.payment.findMany(query);
          const t = await countForPagination(tx.payment, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).payment
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).payment.findMany(query);
          total = await countForPagination(
            (extended as any).payment,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).payment.guard(shape, caller).findMany(query)
        : await (extended as any).payment.findMany(query);
      total = await countForPagination(
        (extended as any).payment,
        query,
