...
export function ProductImageDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("ProductImage", config));
    }
    ...
    const pageTitle = config.docsTitle || `ProductImage API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("ProductImage", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("ProductImage", config);
