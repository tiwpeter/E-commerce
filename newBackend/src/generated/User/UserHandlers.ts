...
export async function UserFindMany(
  ...
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).findMany(query);
    } else {
      data = await (extended as any).user.findMany(query);
    }
...
export async function UserFindFirst(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).findFirst(query);
    } else {
      data = await (extended as any).user.findFirst(query);
    }
...
export async function UserFindUnique(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user
        .guard(shape, caller)
        .findUnique(query);
    } else {
      data = await (extended as any).user.findUnique(query);
    }
...
export async function UserFindUniqueOrThrow(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).user.findUniqueOrThrow(query);
    }
...
export async function UserFindFirstOrThrow(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).user.findFirstOrThrow(query);
    }
...
export async function UserCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).user.count(query);
    }
...
export async function UserAggregate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).aggregate(query);
    } else {
      data = await (extended as any).user.aggregate(query);
    }
...
export async function UserGroupBy(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).groupBy(query);
    } else {
      data = await (extended as any).user.groupBy(query);
    }
...
export async function UserCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).user.create(body);
    }
...
export async function UserCreateMany(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).createMany(body);
    } else {
      data = await (extended as any).user.createMany(body);
    }
...
export async function UserCreateManyAndReturn(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).user.createManyAndReturn(body);
    }
...
export async function UserUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).user.update(body);
    }
...
export async function UserUpdateMany(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).updateMany(body);
    } else {
      data = await (extended as any).user.updateMany(body);
    }
...
export async function UserUpdateManyAndReturn(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).user.updateManyAndReturn(body);
    }
...
export async function UserDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).user.delete(body);
    }
...
export async function UserDeleteMany(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).deleteMany(body);
    } else {
      data = await (extended as any).user.deleteMany(body);
    }
...
export async function UserUpsert(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).user);
      const caller = res.locals.guardCaller;
      data = await (extended as any).user.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).user.upsert(body);
    }
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.user.guard(shape, caller).findMany(query)
            : await tx.user.findMany(query);
          const t = await countForPagination(tx.user, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).user.guard(shape, caller).findMany(query)
            : await (extended as any).user.findMany(query);
          total = await countForPagination(
            (extended as any).user,
            query,
            shape,
            caller,
          );
