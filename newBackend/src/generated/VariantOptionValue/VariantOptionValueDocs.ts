...
const COMPOUND_UNIQUES: { name: string; fields: string[] }[] = [
  { name: "variantId_optionValueId", fields: ["variantId", "optionValueId"] },
];

const EXAMPLE_VALUES: Record<string, unknown> = {
  id: "example",
  variantId: "example",
  optionValueId: "example",
};
...
export function VariantOptionValueDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res
        .type("html")
        .send(renderPlayground("VariantOptionValue", config));
    }
    ...
    const pageTitle = config.docsTitle || `VariantOptionValue API`;
    ...
    const html = renderDocs("VariantOptionValue", config);
