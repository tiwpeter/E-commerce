...
export async function ReviewFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).review.findMany(query);
    }
...
export async function ReviewFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.findFirst(query);
    }
...
export async function ReviewFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.findUnique(query);
    }
...
export async function ReviewFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.findUniqueOrThrow(query);
    }
...
export async function ReviewFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.findFirstOrThrow(query);
    }
...
export async function ReviewCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).review.count(query);
    }
...
export async function ReviewAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.aggregate(query);
    }
...
export async function ReviewGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).groupBy(query);
    } else {
      data = await (extended as any).review.groupBy(query);
    }
...
export async function ReviewCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).review.create(body);
    }
...
export async function ReviewCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.createMany(body);
    }
...
export async function ReviewCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.createManyAndReturn(body);
    }
...
export async function ReviewUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).review.update(body);
    }
...
export async function ReviewUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.updateMany(body);
    }
...
export async function ReviewUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.updateManyAndReturn(body);
    }
...
export async function ReviewDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).review.delete(body);
    }
...
export async function ReviewDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).review.deleteMany(body);
    }
...
export async function ReviewUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).review);
      const caller = res.locals.guardCaller;
      data = await (extended as any).review.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).review.upsert(body);
    }
...
export async function ReviewFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.review.guard(shape, caller).findMany(query)
            : await tx.review.findMany(query);
          const t = await countForPagination(tx.review, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).review
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).review.findMany(query);
          total = await countForPagination(
            (extended as any).review,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).review.guard(shape, caller).findMany(query)
        : await (extended as any).review.findMany(query);
      total = await countForPagination(
        (extended as any).review,
        query,
