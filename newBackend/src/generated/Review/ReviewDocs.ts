...
const COMPOUND_UNIQUES: { name: string; fields: string[] }[] = [
  { name: "userId_productId", fields: ["userId", "productId"] },
];
...
export function ReviewDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Review", config));
    }
    ...
    const pageTitle = config.docsTitle || `Review API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Review", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Review", config);
