...
export function OrderItemDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("OrderItem", config));
    }
    ...
    const pageTitle = config.docsTitle || `OrderItem API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("OrderItem", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("OrderItem", config);
