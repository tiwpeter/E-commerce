...
export async function CategoryFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).category.findMany(query);
    }
...
export async function CategoryFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.findFirst(query);
    }
...
export async function CategoryFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.findUnique(query);
    }
...
export async function CategoryFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.findUniqueOrThrow(query);
    }
...
export async function CategoryFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.findFirstOrThrow(query);
    }
...
export async function CategoryCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).category.count(query);
    }
...
export async function CategoryAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.aggregate(query);
    }
...
export async function CategoryGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.groupBy(query);
    }
...
export async function CategoryCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).category.create(body);
    }
...
export async function CategoryCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.createMany(body);
    }
...
export async function CategoryCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.createManyAndReturn(body);
    }
...
export async function CategoryUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).category.update(body);
    }
...
export async function CategoryUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.updateMany(body);
    }
...
export async function CategoryUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.updateManyAndReturn(body);
    }
...
export async function CategoryDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).category.delete(body);
    }
...
export async function CategoryDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).category.deleteMany(body);
    }
...
export async function CategoryUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).category);
      const caller = res.locals.guardCaller;
      data = await (extended as any).category.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).category.upsert(body);
    }
...
export async function CategoryFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.category.guard(shape, caller).findMany(query)
            : await tx.category.findMany(query);
          const t = await countForPagination(tx.category, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).category
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).category.findMany(query);
          total = await countForPagination(
            (extended as any).category,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).category.guard(shape, caller).findMany(query)
        : await (extended as any).category.findMany(query);
      total = await countForPagination(
        (extended as any).category,
        query,
