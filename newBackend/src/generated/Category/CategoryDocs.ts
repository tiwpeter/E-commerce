...
export function CategoryDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Category", config));
    }
    ...
    const pageTitle = config.docsTitle || `Category API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Category", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Category", config);
