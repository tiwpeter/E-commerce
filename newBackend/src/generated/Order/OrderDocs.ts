...
export function OrderDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Order", config));
    }
    ...
    const pageTitle = config.docsTitle || `Order API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Order", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Order", config);
