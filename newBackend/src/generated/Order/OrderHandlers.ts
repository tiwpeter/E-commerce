...
export async function OrderFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).findMany(query);
    } else {
      data = await (extended as any).order.findMany(query);
    }
...
export async function OrderFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order
        .guard(shape, caller)
        .findFirst(query);
    } else {
      data = await (extended as any).order.findFirst(query);
    }
...
export async function OrderFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.findUnique(query);
    }
...
export async function OrderFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.findUniqueOrThrow(query);
    }
...
export async function OrderFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.findFirstOrThrow(query);
    }
...
export async function OrderCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).order.count(query);
    }
...
export async function OrderAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.aggregate(query);
    }
...
export async function OrderGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).groupBy(query);
    } else {
      data = await (extended as any).order.groupBy(query);
    }
...
export async function OrderCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).order.create(body);
    }
...
export async function OrderCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.createMany(body);
    }
...
export async function OrderCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.createManyAndReturn(body);
    }
...
export async function OrderUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).order.update(body);
    }
...
export async function OrderUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.updateMany(body);
    }
...
export async function OrderUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.updateManyAndReturn(body);
    }
...
export async function OrderDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).order.delete(body);
    }
...
export async function OrderDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).order.deleteMany(body);
    }
...
export async function OrderUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).order);
      const caller = res.locals.guardCaller;
      data = await (extended as any).order.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).order.upsert(body);
    }
...
export async function OrderFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.order.guard(shape, caller).findMany(query)
            : await tx.order.findMany(query);
          const t = await countForPagination(tx.order, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).order.guard(shape, caller).findMany(query)
            : await (extended as any).order.findMany(query);
          total = await countForPagination(
            (extended as any).order,
            query,
            shape,
            caller,
          );
