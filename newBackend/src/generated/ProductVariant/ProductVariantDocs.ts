...
export function ProductVariantDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("ProductVariant", config));
    }
    ...
    const pageTitle = config.docsTitle || `ProductVariant API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("ProductVariant", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("ProductVariant", config);
