...
export function ShippingMethodDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("ShippingMethod", config));
    }
    ...
    const pageTitle = config.docsTitle || `ShippingMethod API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("ShippingMethod", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("ShippingMethod", config);
