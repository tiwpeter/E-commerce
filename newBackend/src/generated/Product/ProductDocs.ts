...
export function ProductDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Product", config));
    }
    ...
    const pageTitle = config.docsTitle || `Product API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Product", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Product", config);
