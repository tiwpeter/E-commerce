...
export async function ProductFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).product.findMany(query);
    }
...
export async function ProductFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.findFirst(query);
    }
...
export async function ProductFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.findUnique(query);
    }
...
export async function ProductFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.findUniqueOrThrow(query);
    }
...
export async function ProductFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.findFirstOrThrow(query);
    }
...
export async function ProductCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).product.count(query);
    }
...
export async function ProductAggregate(
  ...
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.aggregate(query);
    }
...
export async function ProductGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.groupBy(query);
    }
...
export async function ProductCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).product.create(body);
    }
...
export async function ProductCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.createMany(body);
    }
...
export async function ProductCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.createManyAndReturn(body);
    }
...
export async function ProductUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).product.update(body);
    }
...
export async function ProductUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.updateMany(body);
    }
...
export async function ProductUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.updateManyAndReturn(body);
    }
...
export async function ProductDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).product.delete(body);
    }
...
export async function ProductDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).product.deleteMany(body);
    }
...
export async function ProductUpsert(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).product);
      const caller = res.locals.guardCaller;
      data = await (extended as any).product.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).product.upsert(body);
    }
...
export async function ProductFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.product.guard(shape, caller).findMany(query)
            : await tx.product.findMany(query);
          const t = await countForPagination(tx.product, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).product
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).product.findMany(query);
          total = await countForPagination(
            (extended as any).product,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).product.guard(shape, caller).findMany(query)
        : await (extended as any).product.findMany(query);
      total = await countForPagination(
        (extended as any).product,
        query,
