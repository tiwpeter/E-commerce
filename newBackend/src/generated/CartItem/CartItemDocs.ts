...
const COMPOUND_UNIQUES: { name: string; fields: string[] }[] = [
  {
    name: "cartId_productId_variantId",
...
export function CartItemDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("CartItem", config));
    }
    ...
    const pageTitle = config.docsTitle || `CartItem API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("CartItem", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("CartItem", config);
