...
export async function CartItemFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem
        .guard(shape, caller)
        .findMany(query);
    } else {
      data = await (extended as any).cartItem.findMany(query);
    }
...
export async function CartItemFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.findFirst(query);
    }
...
export async function CartItemFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.findUnique(query);
    }
...
export async function CartItemFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.findUniqueOrThrow(query);
    }
...
export async function CartItemFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.findFirstOrThrow(query);
    }
...
export async function CartItemCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).cartItem.count(query);
    }
...
export async function CartItemAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.aggregate(query);
    }
...
export async function CartItemGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.groupBy(query);
    }
...
export async function CartItemCreate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).cartItem.create(body);
    }
...
export async function CartItemCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.createMany(body);
    }
...
export async function CartItemCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.createManyAndReturn(body);
    }
...
export async function CartItemUpdate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).cartItem.update(body);
    }
...
export async function CartItemUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.updateMany(body);
    }
...
export async function CartItemUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.updateManyAndReturn(body);
    }
...
export async function CartItemDelete(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).cartItem.delete(body);
    }
...
export async function CartItemDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cartItem.deleteMany(body);
    }
...
export async function CartItemUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cartItem);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cartItem.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).cartItem.upsert(body);
    }
...
export async function CartItemFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.cartItem.guard(shape, caller).findMany(query)
            : await tx.cartItem.findMany(query);
          const t = await countForPagination(tx.cartItem, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).cartItem
                .guard(shape, caller)
                .findMany(query)
            : await (extended as any).cartItem.findMany(query);
          total = await countForPagination(
            (extended as any).cartItem,
            query,
            shape,
            caller,
          );
      ...
      items = shape
        ? await (extended as any).cartItem.guard(shape, caller).findMany(query)
        : await (extended as any).cartItem.findMany(query);
      total = await countForPagination(
        (extended as any).cartItem,
        query,
