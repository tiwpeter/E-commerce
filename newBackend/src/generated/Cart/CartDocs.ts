...
export function CartDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("Cart", config));
    }
    ...
    const pageTitle = config.docsTitle || `Cart API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("Cart", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("Cart", config);
