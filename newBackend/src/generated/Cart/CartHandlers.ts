...
export async function CartFindMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    let data;
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).findMany(query);
    } else {
      data = await (extended as any).cart.findMany(query);
    }
...
export async function CartFindFirst(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).findFirst(query);
    } else {
      data = await (extended as any).cart.findFirst(query);
    }
...
export async function CartFindUnique(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart
        .guard(shape, caller)
        .findUnique(query);
    } else {
      data = await (extended as any).cart.findUnique(query);
    }
...
export async function CartFindUniqueOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cart.findUniqueOrThrow(query);
    }
...
export async function CartFindFirstOrThrow(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cart.findFirstOrThrow(query);
    }
...
export async function CartCount(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).count(query);
    } else {
      data = await (extended as any).cart.count(query);
    }
...
export async function CartAggregate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).aggregate(query);
    } else {
      data = await (extended as any).cart.aggregate(query);
    }
...
export async function CartGroupBy(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).groupBy(query);
    } else {
      data = await (extended as any).cart.groupBy(query);
    }
...
export async function CartCreate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).create(body);
    } else {
      data = await (extended as any).cart.create(body);
    }
...
export async function CartCreateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).createMany(body);
    } else {
      data = await (extended as any).cart.createMany(body);
    }
...
export async function CartCreateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cart.createManyAndReturn(body);
    }
...
export async function CartUpdate(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).update(body);
    } else {
      data = await (extended as any).cart.update(body);
    }
...
export async function CartUpdateMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).updateMany(body);
    } else {
      data = await (extended as any).cart.updateMany(body);
    }
...
export async function CartUpdateManyAndReturn(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      ...
      data = await (extended as any).cart.updateManyAndReturn(body);
    }
...
export async function CartDelete(
  ...
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).delete(body);
    } else {
      data = await (extended as any).cart.delete(body);
    }
...
export async function CartDeleteMany(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).deleteMany(body);
    } else {
      data = await (extended as any).cart.deleteMany(body);
    }
...
export async function CartUpsert(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (shape) {
      assertGuard((extended as any).cart);
      const caller = res.locals.guardCaller;
      data = await (extended as any).cart.guard(shape, caller).upsert(body);
    } else {
      data = await (extended as any).cart.upsert(body);
    }
...
export async function CartFindManyPaginated(
  req: Request,
  res: Response,
  next: NextFunction,
) {
    ...
    if (typeof extended.$transaction === "function") {
      try {
        const txResult = await extended.$transaction(async (tx: any) => {
          const d = shape
            ? await tx.cart.guard(shape, caller).findMany(query)
            : await tx.cart.findMany(query);
          const t = await countForPagination(tx.cart, query, shape, caller);
          return { d, t };
        });
        items = txResult.d;
        total = txResult.t;
        ...
        if (
          ...
          items = shape
            ? await (extended as any).cart.guard(shape, caller).findMany(query)
            : await (extended as any).cart.findMany(query);
          total = await countForPagination(
            (extended as any).cart,
            query,
            shape,
            caller,
          );
