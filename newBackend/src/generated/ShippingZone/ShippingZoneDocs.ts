...
export function ShippingZoneDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("ShippingZone", config));
    }
    ...
    const pageTitle = config.docsTitle || `ShippingZone API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("ShippingZone", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("ShippingZone", config);
