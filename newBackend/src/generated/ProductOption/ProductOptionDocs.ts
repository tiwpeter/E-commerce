...
const COMPOUND_UNIQUES: { name: string; fields: string[] }[] = [
  { name: "productId_name", fields: ["productId", "name"] },
];
...
export function ProductOptionDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("ProductOption", config));
    }
    ...
    const pageTitle = config.docsTitle || `ProductOption API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(
          renderScalar("ProductOption", spec, pageTitle, config.scalarCdnUrl),
        );
    }

    const html = renderDocs("ProductOption", config);
